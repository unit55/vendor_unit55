function __print_unit55_functions_help() {
cat <<EOF
...
- githubremote:    Add git remote for Prime Github.
...
{
    if [ "$OUT" ] ; then
        ZIPPATH=`ls -tr "$OUT"/Prime-*.zip | tail -1`
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
        fi
        ...
        echo "Found device"
        if (adb shell getprop com.unit55.device | grep -q "$UNIT55_BUILD"); then
            echo "Rebooting to sideload for install"
            adb reboot sideload-auto-reboot
            adb wait-for-sideload
            adb sideload $ZIPPATH
        else
            echo "The connected device does not appear to be $UNIT55_BUILD, run away!"
        fi
        return $?
    else
        echo "Nothing to eat"
...
function githubremote()
{
    ...
    local REMOTE=$(git config --get remote.unit55.projectname)

    local PROJECT=$(echo $REMOTE | sed -e "s#platform/#android/#g; s#/#_#g")

    git remote add github https://github.com/RealSchnedi/$PROJECT
...
function installboot()
{
    ...
    adb root
    adb wait-for-device-recovery
    if (adb shell getprop org.unit55.device | grep -q "$UNIT55_BUILD");
    then
...
function dopush()
{
    ...
    if (adb shell getprop com.unit55.device | grep -q "$UNIT55_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
